...
BASE_DIR="/data_2/t2vid-data/benchmarks/MLVU"
...
LOG=${OUTPUT_DIR}/eval_mlvu_${NUM_FRAMES}frame.log
...
for IDX in $(seq 0 $((CHUNKS-1))); do
    CUDA_VISIBLE_DEVICES=${GPULIST[$IDX]} python -m model_video_qa_loader \
        --model-path $CKPTFILE \
        --question-file ${BASE_DIR}/mlvu.jsonl \
...
LOG_1=${OUTPUT_DIR}/eval_result_mlvu_${NUM_FRAMES}frame.log 
# Clear out the log file if it exists.
> "${LOG_1}"
exec &> >(tee -a "${LOG_1}")
...
for IDX in $(seq 0 $((CHUNKS-1))); do
   CUDA_VISIBLE_DEVICES=${GPULIST[$IDX]} python -u parse_answer_with_llm.py \
       ...
       --output-file ${OUTPUT_DIR}/mlvu/results/${CHUNKS}_${IDX}.jsonl \
       --num-chunks $CHUNKS \
       --chunk-idx $IDX &
done
...
new_output_file=${OUTPUT_DIR}/mlvu/results/merge.jsonl
...
# Loop through the indices and concatenate each file.
for IDX in $(seq 0 $((CHUNKS-1))); do
    cat ${OUTPUT_DIR}/mlvu/results/${CHUNKS}_${IDX}.jsonl >> "$new_output_file"
done
